...
def test_torguapi_json_1():
    """torguapi_json basic test"""
    body = {"foo": "bar"}
    status_code = 200
    result = torguapi.torguapi_json(status_code, body)
...
def test_torguapi_json_2():
    """torguapi_json raised excepiton if body is None"""
    body = None
    status_code = 200
    with pytest.raises(torguapi.TorguapiError):
        torguapi.torguapi_json(status_code, body)

def test_torguapi_json_3():
    """torguapi_json raised excepiton if status_code is None"""
    body = {"foo": "bar"}
    status_code = None
    with pytest.raises(torguapi.TorguapiError):
        torguapi.torguapi_json(status_code, body)
...
def test_make_links_and_meta_1():
    """make_links_and_meta basic test"""
    url_base = "https://example.com"
    aux = {"page_number": 3, "record_count": 101, "page_size": 10}
    links, meta = torguapi.make_links_and_meta(aux, url_base)
    expected_links = {
        "self": "https://example.com?page_number=3&page_size=10",
        "next": "https://example.com?page_number=4&page_size=10",
        "previous": "https://example.com?page_number=2&page_size=10"
    }
    expected_meta = {
        "page_count": 11,
        "record_count": 101
...
def test_get_page_parameters_1():
    """get_page_parameters use default query parameters"""
    query_parameters = {"a":"b"}
    params = torguapi.get_page_parameters(query_parameters)
    expected_params = {"page_size": 100}
    assert expected_params == params

def test_get_page_parameters_2():
    """get_page_parameters basic test"""
    query_parameters = {"a":"b", "page_size": "37", "page_number": "2"}
    params = torguapi.get_page_parameters(query_parameters)
    expected_params = {"page_size": 37, "page_number": 2}
    assert expected_params == params    

def test_get_page_parameters_3():
    """get_page_parameters invalid page size (non-numeric)"""
    query_parameters = {"a":"b", "page_size": "foo", "page_number": "2"}
    with pytest.raises(torguapi.TorguapiInvalidRequest):
        torguapi.get_page_parameters(query_parameters)

def test_get_page_parameters_4():
    """get_page_parameters invalid page size (less than 1)"""
    query_parameters = {"a":"b", "page_size": "0", "page_number": "2"}
    with pytest.raises(torguapi.TorguapiInvalidRequest):
        torguapi.get_page_parameters(query_parameters)

def test_get_page_parameters_5():
    """get_page_parameters invalid page number"""
