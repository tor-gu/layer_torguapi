...
def torguapi_json(status_code, body):
...
def torguapi_http_error(status_code, detail=None):
    ...
    if detail is not None:
        error["detail"] = detail
    errors = {"errors": [error]}
    return torguapi_json(status_code, errors)
...
def torguapi_result(result, links, meta=None):
    ...
    if meta is not None and len(meta) > 0:
        body["meta"] = meta
    return torguapi_json(HTTPStatus.OK, body)            
...
def make_links_and_meta(aux, url_base):
    calculate_page_count(aux)
    links = make_page_links(aux, url_base)
    meta = make_meta(aux)
    return links, meta

def get_page_parameters(query_parameters):
    page_params = {}
